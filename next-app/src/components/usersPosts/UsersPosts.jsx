"use client";
import { useEffect, useState } from "react";
import CommentSection from "../commentSection/commentSection";
import ShowBtn from "../showBtn/ShowBtn";
import styles from "./UsersPosts.module.css";
import {
  deleteRequest,
  getPostRequest,
  postLikeRequest,
  userPermsRequest,
} from "@/api/post";
import { useAuth } from "@/app/context/AuthContext";

export default function UsersPosts() {
  const [posts, setPosts] = useState([]);
  const [userRole, setUserRole] = useState(null);
  const { user } = useAuth();

  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await getPostRequest();
        const data = await response.json();
        setPosts(data);
      } catch (error) {
        console.error("Error fetching posts:", error);
      }
    };

    const checkUserRole = async (userId) => {
      try {
        const response = await userPermsRequest(userId);
        const data = await response.json();

        if (data.rol === 0) {
          console.log("<--- Soy un Usuario --->");
          setUserRole(data.rol);
        } else if (data.rol === 1) {
          console.log("<--- Soy un Administrador --->");
          setUserRole(data.rol);
        }
      } catch (error) {
        console.error("Error:", error);
      }
    };

    fetchPosts();
    if (user && user.id) {
      checkUserRole(user.id);
    }
    const pollingInterval = 2000; // Fetch every 5 seconds

    const intervalId = setInterval(fetchPosts, pollingInterval);

    return () => {
      clearInterval(intervalId); // Clean up the interval on unmount
    };
  }, [user, user.id,]);

  

  const deletePost = async (postId) => {
    try {
      await deleteRequest(postId);
      // Perform additional actions or updates after deletion if needed
    } catch (error) {
      console.error("Error deleting post:", error);
    }
  };

  const handleLikeBtn = async (postId) => {
    try {
      // Find the post that corresponds to the postId
      const postToUpdate = posts.find((post) => post._id === postId);
  
      // Calculate the updated likes count
      const updatedLikes = postToUpdate.liked
        ? postToUpdate.likes - 1
        : postToUpdate.likes + 1;
  
      // Update the local state first
      setPosts((prevPosts) =>
        prevPosts.map((prevPost) =>
          prevPost._id === postId
            ? { ...prevPost, liked: !prevPost.liked, likes: updatedLikes }
            : prevPost
        )
      );
  
      // Send the like request to the server
      await postLikeRequest(postId, { likes: updatedLikes });
    } catch (error) {
      console.error("Error liking post:", error);
    }
  };
  

  return (
    <>
      {posts.map((post) => (
        <div className={styles.userPost} key={post._id}>
          <div className={styles.userInfo}>
            <img src={post.photo} alt="" />
            <div className={styles.userData}>
              <div className={styles.nameDate}>
                <span className={styles.userName}>{post.name}</span>
              </div>
              <span className={styles.userLocation}>{post.createdAt}</span>
            </div>
            <div className={styles.postOptions}>
              <div className={styles.postLikes}>
                <button
                  className={styles.likeBtn}
                  onClick={() => handleLikeBtn(post._id)}
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
                    strokeWidth={1.5}
                    stroke="currentColor"
                    className="w-6 h-6"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z"
                    />
                  </svg>
                </button>
                <span>{post.likes}</span>
              </div>
              {userRole === 0 ? (
                ""
              ) : userRole === 1 ? (
                <button className={styles.trashBtn}  onClick={() => deletePost(post._id)}>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
                    strokeWidth={1.5}
                    stroke="currentColor"
                    className="w-6 h-6"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0"
                    />
                  </svg>
                </button>
              ) : (
                ""
              )}
            </div>
          </div>
          <section className={styles.postContent}>
            <h3 className={styles.postTitle}>{post.title}</h3>
            <p className={styles.postText}>{post.description}</p>
          </section>
          <section>
            <CommentSection UserPostId={post._id}/>
          </section>
        </div>
      ))}
    </>
  );
}
